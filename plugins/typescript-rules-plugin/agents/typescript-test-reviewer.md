...
スキル `typescript-code-review-skill` の参照ファイル `test-review` を取得し、その基準に基づいてテストコードを評価する。

評価観点:
1. テスト構造と命名（フラット構造推奨、`describe` 不使用）
2. テストの独立性と再現性
3. アサーションの品質
4. モックとスタブの戦略
5. テストの網羅性

詳細な基準は `typescript-code-review-skill:test-review` を参照すること。
